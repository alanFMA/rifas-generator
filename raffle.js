...
const baseImagePath = './bases.png'; // Caminho da imagem base
...
const ticketsPerSheet = 6; // Quantas rifas por folha (6 por folha)
const startNumber = 1; // Número inicial (por exemplo, 101)
const endNumber = 6; // Número final (por exemplo, 200)
...
async function generateRaffleSheets(baseImagePath, outputDir, ticketsPerSheet, startNumber, endNumber) {
  ...
  while (currentNumber <= endNumber) {
    ...
    for (let j = 0; j < ticketsPerSheet; j++) {
      ...
      ctx.fillStyle = 'black';
      ctx.textAlign = 'left';
      ctx.fillText(formattedNumber, x + 300, y + 140);

      // Adicionar o número da rifa na área do "NÚMERO DO BILHETE:"
      ctx.font = 'bold 70px Arial';
      ctx.textAlign = 'left';
      ctx.fillText(formattedNumber, x + 900, y + ticketHeight - 150);
...
generateRaffleSheets(baseImagePath, outputDir, ticketsPerSheet, startNumber, endNumber);
